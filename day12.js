...
class Context {
  ...
  onClose() {
    let startIndexes = this.part == 1 ? this.partOneStartPoints : this.partTwoStartPoints;
    console.log(startIndexes.length, startIndexes);
    const stepsTaken = this.heightMap.mapDistanceFrom(startIndexes);
    console.log('Distance to endpoint:', this.heightMap.endCell.distance);
    console.log('Steps taken:', stepsTaken);
  }


  get partOneStartPoints() {
    return [this.heightMap.startIndex];
  }

  get partTwoStartPoints() {
    return this.heightMap.grid
      .map((c, i) => ({ idx: i, cell: c }))
      .filter(o => o.cell.name === 'b')
      .map(o => this.heightMap.neighborsOnMap(o.idx).filter(idx => this.heightMap.grid[idx].height === 0))
      .flat();
  }
...
var part = processModule.argv.length === 2 ? 1 : processModule.argv[2];
